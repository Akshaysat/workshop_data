...
def registered_data(workshop_id, offset=1, df1=0):
    ...
    if offset == 1:
        url = (
            "https://catalog.prod.learnapp.com/kraken/catalog/workshops/"
            + workshop_id
            + "/registrations"
        )
...
def get_workshop_id(workshop_canonical_title):
    print(token)
    url = (
        "https://catalog.prod.learnapp.com/kraken/catalog/workshops/titles/"
        + workshop_canonical_title
    )
    payload = {}
    headers = {
        "authority": "catalog.prod.learnapp.com",
